...
import { Video, VideoOff, Mic, MicOff, Monitor, MonitorOff, Users, MessageSquare, Send, X, LogOut, Loader2 } from 'lucide-react'
...
const AgoraLiveStream = () => {
  ...
  const initializeFromStorage = async () => {
    try {
      ...
      const storedChannelName = localStorage.getItem('sessionName')
      const storedToken = localStorage.getItem('agoraToken')
      const storedUid = localStorage.getItem('userId')

      if (!storedUserName || !storedChannelName || !storedToken || !storedUid) {
        setError('Missing session data. Please ensure you have: sessionUserName, sessionName, agoraToken, and userid in localStorage')
        setLoading(false)
        return
      }
      ...
      // Auto join after getting data
      await joinChannelAuto(storedUserName, storedChannelName, storedToken, Number(storedUid))
  ...
  const joinChannelAuto = async (name: string, channel: string, tkn: string, userId: number) => {
    try {
      ...
      await client.join(APP_ID, channel, tkn, userId)
      ...
      client.on('stream-message', (uid: number, data: any) => {
        try {
          ...
          if (message.type === 'user-info') {
            ...
            if (!showChat && message.uid !== userId) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-red-700 mb-2">Joining Session...</h2>
          <p className="text-gray-600">Please wait while we connect you to the live stream</p>
        </div>
      </div>
    )
  }
  ...
  if (error && !joined) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="bg-red-50 border-2 border-red-700 rounded-lg p-8 max-w-md text-center">
