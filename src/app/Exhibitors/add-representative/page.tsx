...
const ExhibitorRepresentatives: React.FC = () => {
  ...
  const fetchRepresentatives = async () => {
    ...
    try {
      const response = await api.get(`/exhibitor-representatives/exhibitor/${exhibitorId}`);
  ...
  const handleAddRepresentative = async () => {
    if (!exhibitorId || !selectedUserId || !displayTitle) return;
    setLoadingAdd(true);
    try {
      await api.post('/exhibitor-representatives', {
        exhibitorId,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/exhibitor-representatives/${id}`);
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-4 gap-8 bg-gray-50">
      ...
      <div className="bg-white border border-gray-300 rounded-2xl shadow-lg p-10 w-full max-w-lg flex flex-col gap-4">
          ...
          <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-xl">
            {filteredParticipants.map(user => (
              <div
                ...
                className={`flex items-center gap-3 p-3 cursor-pointer ${selectedUserId === user.id ? 'bg-red-100' : 'hover:bg-gray-100'}`}
              >
                <img
            ...
            {filteredParticipants.length === 0 && <div className="p-3 text-gray-500">No participants found</div>}
          </div>
        ...
        <Link href="/exhibitors/manage" className="py-2 px-6 bg-white text-red-900 border-2 border-red-900 rounded-xl flex items-center justify-center gap-2 hover:bg-red-900 hover:text-white cursor-pointer">
...
export default ExhibitorRepresentatives;
