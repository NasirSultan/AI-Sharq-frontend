...
export default function MyAgendaPage() {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      const filteredAllSessions = (data.allSessions || []).filter((s: any) => !liveIds.includes(s.sessionId))
      const combined = [...(data.liveSessions || []), ...filteredAllSessions]
      const transformed = combined.map((s: any) => ({ ...s, ...parseDuration(s.duration || "") }))
            ...
            >
              ...
              <div className="flex flex-col gap-2 mt-auto">
                <Link href={session?.sessionId ? `/participants/SessionDetail1/${session.sessionId}` : "#"}>
                  <button className="w-full bg-[#9B2033] text-white py-3 text-sm font-medium rounded-lg hover:bg-red-700 transition-colors duration-200 shadow-sm cursor-pointer">
                    View Details
                  </button>
                </Link>
