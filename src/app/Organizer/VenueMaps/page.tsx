    ...
    <div className="min-h-screen bg-[#FAFAFA] px-4 md:px-8 lg:px-12 py-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full">
        {[
          ...
          <div key={idx} className="flex-1 flex items-center p-4 sm:p-5 bg-white border border-[#E6E6E6] shadow rounded-2xl min-w-[180px]">
            <div className={`w-12 h-12 ${stat.bg} rounded-2xl flex items-center justify-center`}>{stat.icon}</div>
            <div className="flex flex-col ml-3 sm:ml-4">
              <span className="font-semibold text-2xl sm:text-3xl text-[rgba(0,0,0,0.7)]">{stat.value}</span>
              <span className="text-sm sm:text-base text-[#414141]">{stat.label}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    Please make sure the current event record is complete. Only one event can exist at a time and a new event will replace the previous one.
      ...
      <div className="flex flex-col gap-6 w-full">
        {data.events.map((event) => (
          <div key={event.id} className="flex flex-col md:flex-row justify-between p-4 sm:p-6 gap-4 md:gap-6 w-full bg-white border border-[#D4D4D4] shadow rounded-2xl">
            ...
            <div className="flex flex-col md:items-end gap-2 sm:gap-3 mt-3 md:mt-0">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                <button
                  className="px-4 sm:px-8 py-1 bg-[#9B2033] text-white rounded-2xl hover:bg-[#7e0505] transition"
                ...
                <button
                  className="px-4 sm:px-8 py-1 border border-[#8C8C8C] rounded-2xl hover:bg-gray-50 transition"
